...
import os
import math
from classification import training, getLabel, CLASS_NUMBER

SIGNS = ["DO NOT TURN LEFT",
        "NO ENTRY",
        "NO HORN",
         "NO PARKING",
         "NO RICKSHAW",
...
SIMILARITY_CONTOUR_WITH_CIRCLE = 0.7
...
def preprocess_image(image):
    ...
    image = cv2.GaussianBlur(image, (3, 3), 0)  # paramter
    image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    image = cv2.Laplacian(image, cv2.CV_8U, 3, 3, 2)  # parameter
...
def localization(image, min_size_components, similitary_contour_with_circle, model, count, current_sign_type):
    ...
    if sign is not None:
        ...
        sign_type = sign_type if sign_type <= SIZE else SIZE
...
def main(FILE_NAME, MIN_SIZE_COMPONENTS, SIMILARITY_CONTOUR_WITH_CIRCLE):
    ...
    while True:
        ...
        coordinate, image, sign_type, text = localization(frame, MIN_SIZE_COMPONENTS, SIMILARITY_CONTOUR_WITH_CIRCLE, model, count, current_sign)
        ...
        if sign_type >= 0 and (not current_sign or sign_type != current_sign):
            ...
            position = [count, sign_type if sign_type <= SIZE  else SIZE, coordinate[0][0], coordinate[0][1], coordinate[1][0], coordinate[1][1]]
            ...
            if tl[0] >= 0 and tl[1] >= 0 and br[0] >= 0 and br[1] >= 0 and roi.shape[0] > 0 and roi.shape[1] > 0 and roi.shape[2] > 0:
            ...
            if sign_type >= 0:
                ...
                position = [count, sign_type if sign_type <= SIZE else SIZE, left, top, right, bottom]
                cv2.rectangle(image, coordinate[0],coordinate[1], (0, 255, 0), 1)
                font = cv2.FONT_HERSHEY_PLAIN
                cv2.putText(image,text,(coordinate[0][0], coordinate[0][1] -15), font, 1,(0,0,255),2,cv2.LINE_4)
            elif current_sign:
                position = [count, sign_type if sign_type <= SIZE else SIZE, tl[0], tl[1], br[0], br[1]]
...
main(FILE_NAME, MIN_SIZE_COMPONENTS, SIMILARITY_CONTOUR_WITH_CIRCLE)